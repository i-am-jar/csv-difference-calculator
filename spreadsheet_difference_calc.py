...
def calculate_differences(df1, df2, column_name, column_sub, column_new, file_name):
    ...
    # Check if columns contain non-string data before applying string operations
    if merged_df[column_sub+'_1'].dtype == 'object':
        merged_df[column_sub+'_1'] = pd.to_numeric(merged_df[column_sub+'_1'].str.replace(',', ''), errors='coerce')
    else:
        merged_df[column_sub+'_1'] = pd.to_numeric(merged_df[column_sub+'_1'], errors='coerce')

    if merged_df[column_sub+'_2'].dtype == 'object':
        merged_df[column_sub+'_2'] = pd.to_numeric(merged_df[column_sub+'_2'].str.replace(',', ''), errors='coerce')
    else:
        merged_df[column_sub+'_2'] = pd.to_numeric(merged_df[column_sub+'_2'], errors='coerce')
